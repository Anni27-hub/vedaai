...
export default function CreateAssignmentPage() {
  ...
  const validate = () => {
    if (!store.institutionName.trim()) return 'Please add your college or institution name.'
    if (!store.courseName.trim()) return 'Please add the course name.'
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      fd.append('institutionName', store.institutionName)
      fd.append('courseName', store.courseName)
      fd.append('examTitle', store.examTitle || 'Question Paper')
        ...
        <div className="max-w-2xl space-y-6">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-2">College / Institution Name</label>
                <input
                  className="input"
                  placeholder="e.g. VIT Bhopal University"
                  value={store.institutionName}
                  onChange={e => store.setInstitutionName(e.target.value)}
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-2">Course / Subject</label>
                <input
                  className="input"
                  placeholder="e.g. B.Tech CSE - Operating Systems"
                  value={store.courseName}
                  onChange={e => store.setCourseName(e.target.value)}
                />
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-medium text-gray-700 mb-2">Exam Title</label>
                <input
                  className="input"
                  placeholder="e.g. Mid Semester Assessment"
                  value={store.examTitle}
                  onChange={e => store.setExamTitle(e.target.value)}
                />
              </div>
            </div>
