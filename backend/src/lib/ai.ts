...
export async function generateToolkitOutput(
  tool: 'question-generator' | 'rubric-builder' | 'lesson-planner' | 'feedback-ai' | 'essay-evaluator',
  title: string,
  input: string
): Promise<string> {
  const labels: Record<typeof tool, string> = {
    'question-generator': 'Generate classroom-ready questions with answers',
    'rubric-builder': 'Create a marking rubric with criteria and levels',
    'lesson-planner': 'Create a structured lesson plan',
    'feedback-ai': 'Write constructive teacher feedback',
    'essay-evaluator': 'Evaluate an essay response with score and comments',
  }
  ...
  const completion = await client.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are an expert teaching assistant. Return concise, classroom-ready content in plain text with clear headings.',
      },
      {
        role: 'user',
        content: `${labels[tool]}\nTitle: ${title}\nTeacher input:\n${input}`,
  ...
  return completion.choices[0]?.message?.content?.trim() || 'No output generated.'
}
